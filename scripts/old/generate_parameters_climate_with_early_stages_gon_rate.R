...
temperature = Iquitos.climate$temperature_mean[startIndex:(startIndex + length(female_egg_laying_timeseries_nn6)-1) ]
temp.mean <- mean(temperature,na.rm = TRUE)
temperature[is.na(temperature)] <- temp.mean

eip.mean = array(data = 0, dim = c(length(female_egg_laying_timeseries_nn6),1))
eip.mu = array(data = 0, dim = c(length(female_egg_laying_timeseries_nn6),1))
bite.first = array(data = 0, dim = c(length(female_egg_laying_timeseries_nn6),1))
bite.second = array(data = 0, dim = c(length(female_egg_laying_timeseries_nn6),1))
moz.death = array(data = 0, dim = c(length(female_egg_laying_timeseries_nn6),1))

for (t in 1:length(female_egg_laying_timeseries_nn6)){
...
mosquito.data = data.frame(EIP = eip.mu, firstBite = bite.first, secondBite = bite.second, eggDeath = egg_mortality, larvaeDeath = larval_mortality, pupalDeath = pupal_mortality,  mozDeath = DeathRate, eggDev=egg_development_rate, larvalDev = larval_development_rate, pupalDev = pupal_development_rate, adultDev=female_egg_laying_timeseries_nn6)
