...
beta = 0.302#3.518e-4#
alpha = 0.229*((63*mean(gonotrophic_cycle_rate)^beta))#0.922*((63*mean(gonotrophic_cycle_rate)^beta))#
...
for (i in seq(length(dNdt))) {
  product = (exp(-alpha*adult_timeseries_trend^beta)*adult_timeseries_trend*f_N[i+burn_period,])[1:(length(tvec)-burn_period)] #here we include the early time-points for the integral
...
for (i in seq(2+burn_period, length(emergence)+burn_period)){
  product = exp(-alpha*adult_timeseries_check^beta)*adult_timeseries_check*f_N[i-1,1:(length(tvec)-burn_period)]
...
for (i in seq(2+burn_period, length(emergence)+burn_period)){
  product_1 = exp(-alpha*adult_timeseries_check_1^beta)*adult_timeseries_check_1*f_N[i-1,1:(length(tvec)-burn_period)]
  ...
  product_2 = exp(-alpha*adult_timeseries_check_2^beta)*adult_timeseries_check_2*f_N[i-1,1:(length(tvec)-burn_period)]
  ...
  product_3 = exp(-alpha*adult_timeseries_check_3^beta)*adult_timeseries_check_3*f_N[i-1,1:(length(tvec)-burn_period)]
...
lines(adult_timeseries_check_1[(burn_period+1):length(adult_timeseries_check)], col='red')
lines(adult_timeseries_check_2[(burn_period+1):length(adult_timeseries_check)], col='green')
lines(adult_timeseries_check_3[(burn_period+1):length(adult_timeseries_check)], col='blue')

abs_errors = vector(length=3)
abs_errors[1] = sum(abs(adult_timeseries_trend[(burn_period+1):(length(tvec)-burn_period)] - adult_timeseries_check_1[(burn_period+1):length(adult_timeseries_check)]))
abs_errors[2] = sum(abs(adult_timeseries_trend[(burn_period+1):(length(tvec)-burn_period)] - adult_timeseries_check_2[(burn_period+1):length(adult_timeseries_check)]))
abs_errors[3] = sum(abs(adult_timeseries_trend[(burn_period+1):(length(tvec)-burn_period)] - adult_timeseries_check_3[(burn_period+1):length(adult_timeseries_check)]))
...
#======================================================================================
correction_factors = seq(0.98, 1.0, 0.001)
load("delay_integrals.RData")
...
for (j in seq(length(correction_factors))){
  ...
  for (i in seq(2+burn_period, length(emergence)+burn_period)){
    product = exp(-alpha*adult_timeseries_check_cf_matrix[,j]^beta)*adult_timeseries_check_cf_matrix[,j]*f_N[i-1,1:(length(tvec)-burn_period)]
    emergence_integral_check = sum(product[1:(i-1)] + product[2:i])/2
    adult_timeseries_check_cf_matrix[i,j] = adult_timeseries_check_cf_matrix[i-1,j] + correction_factor*emergence_1[i-burn_period-1]*emergence_integral_check - DeathRate[i-1]*adult_timeseries_check_cf_matrix[i-1,j]
  }
  print(j)
}
...
#==================================================
adult_timeseries_no_spraying = vector(length = length(emergence)+burn_period)
adult_timeseries_no_spraying[1:(burn_period+1)] = adult_timeseries_trend[1:(burn_period+1)]
adult_timeseries_spraying = adult_timeseries_no_spraying

spraying_thoroughness = 0.5
spray_today = vector(length = length(DeathRate))
start_day = burn_period+250; end_day= burn_period+start_day+90
spray_today[start_day:end_day] = TRUE 

for (i in seq(2+burn_period, length(emergence)+burn_period)){
  product_spraying = exp(-alpha*adult_timeseries_spraying^beta)*adult_timeseries_spraying*f_N[i-1,1:(length(tvec)-burn_period)]
  emergence_integral_spraying = sum(product_spraying[1:(i-1)] + product_spraying[2:i])/2
  adult_timeseries_spraying[i] = adult_timeseries_spraying[i-1] + correction_factor*emergence_1[i-burn_period-1]*emergence_integral_spraying - (DeathRate[i-1]+spraying_thoroughness*spray_today[i-1])*adult_timeseries_spraying[i-1]

  product_no_spraying = exp(-alpha*adult_timeseries_no_spraying^beta)*adult_timeseries_no_spraying*f_N[i-1,1:(length(tvec)-burn_period)]
  emergence_integral_no_spraying = sum(product_no_spraying[1:(i-1)] + product_no_spraying[2:i])/2
  adult_timeseries_no_spraying[i] = adult_timeseries_no_spraying[i-1] + correction_factor*emergence_1[i-burn_period-1]*emergence_integral_no_spraying - DeathRate[i-1]*adult_timeseries_no_spraying[i-1]
}

plot(adult_timeseries_spraying, type='l')
lines(adult_timeseries_no_spraying, col='red')
...
burnin_population = data.frame(population = adult_timeseries_trend[(burn_period-integration_period+1):burn_period], densityTerms = exp(-alpha*adult_timeseries_trend^beta)[(burn_period-integration_period+1):burn_period])
write.csv(burnin_population, paste0("../output/", popnFileName), row.names = FALSE, quote = FALSE)
...
writeLines(paste("alpha = ", alpha), fileName)
writeLines(paste("beta = ", beta), fileName)
...
save(emergence_1, DeathRate, correction_factor, burn_period, file="time_dependent_series_delay.RData")
